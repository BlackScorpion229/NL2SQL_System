...
API_PORT = os.getenv("APP_PORT", "8001")
...
def render_response(answer: str, data: Any, metadata: Dict) -> None:
    """
    ...
    with st.sidebar:
        ...
        if not st.session_state.jwt_token:
            ...
            if st.button("🔑 Login", use_container_width=True):
                token = get_jwt_token(role)
                if token:
                    st.session_state.jwt_token = token
                    st.session_state.user_role = role
                    st.session_state.messages = []  # Clear chat on login

                    # Auto-load schema and insights immediately on login
            ...
            # Logged in state
            st.markdown(f'<span class="status-ok">✓ Logged in as {st.session_state.user_role}</span>',
                        unsafe_allow_html=True)
