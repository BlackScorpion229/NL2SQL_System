...
def summarize_results(question: str, columns: List[str], rows: List[List[Any]], row_count: int) -> Dict[str, str]:
    ...
    try:
        ...
        # Create prompt template based on result size and analytical nature
        if row_count <= 1 and len(columns) <= 2 and not is_analytical:
            # Simple scalar result - be concise
            system_prompt = """You are a helpful data assistant for an Indian-based application. 
            
Your task is to provide a direct, concise answer to the user's question based on the query result.
            
Guidelines:
- Give the answer directly in a single sentence.
- Do NOT provide "Key Findings", "Statistical Insights", or "Business Implications".
- Do NOT mention that the data is a "single snapshot" or "limited".
- Just state the fact clearly.

INDIAN LOCALIZATION:
- Represent all monetary amounts in Indian Rupees (₹) unless specified otherwise
- Use Indian number system: thousands, lakhs (1,00,000), crores (1,00,00,000)
- NEVER use millions or billions - use lakhs and crores instead
- Format: ₹1,234 (thousands), ₹12.5 lakhs, ₹5.6 crores
...
"""
            ...
            # Complex result - provide analysis
            stats_section = ""
